"""v1.20 — 5 nested level/career-stage hazards.

Hazards (binary discrete-time, "first time reaching this tier or above"):
  A        : first season at level A- / A / A+ / AA / AAA / MLB
  AA       : first season at level AA / AAA / MLB
  AAA      : first season at level AAA / MLB
  MLB      : first MLB appearance (mlb_debut_year)
  ALL_STAR : first all-star selection

4 lasso-logit TARGETS (predicted from the 5 hazards above):
  TOP_100_PROSPECT, MLB_DEBUT, ESTABLISHED_MLB, All_Star_Plus

Same split convention as v1.19: 80% train / 10% cal / 10% val from v1.17
lasso_fit_players + lasso_val_players. Out-of-fold hazard scoring for the
fit + val long files.
"""
...
# Hazards, listed by trigger-condition (levels at or above this tier).
HAZARDS = ["A", "AA", "AAA", "MLB", "ALL_STAR"]
...
def _first_year_at_levels(db: str, level_set: set[str]) -> dict[str, int]:
    """First season per player at any level in level_set."""
    placeholders = ",".join("?" * len(level_set))
    c = sqlite3.connect(db)
    df = pd.read_sql(
        f"SELECT player_id, season_year, level FROM season_stats "
        f"WHERE UPPER(level) IN ({placeholders})",
        c, params=[lv.upper() for lv in level_set])
    c.close()
...
def _triggers_for_player(p: dict,
                          level_triggers: dict[str, dict[str, int]]
                          ) -> dict[str, int | None]:
    """Return trigger years for all hazards + 4 targets."""
    def _y(k):
        v = p.get(k)
        try:
    ...
    return {
        # hazards
        "A":   level_triggers["A"].get(pid),
        "AA":  level_triggers["AA"].get(pid),
        "AAA": level_triggers["AAA"].get(pid),
        "MLB": debut,
        "ALL_STAR": as1,
        # targets
...
def _per_row_labels(joined: list[dict], years: np.ndarray,
    ...
    for i in range(n):
        p = joined[i]
        yr = int(years[i])
        trigs = _triggers_for_player(p, level_triggers)
        la = last_active_by_pid.get(p["player_id"])
        # Hazard label semantics: eligible if event hasn't triggered yet
        # AND player still has observable future; realized = 1 if event
        # triggered exactly this year.
        for ev in HAZARDS:
            trig = trigs[ev]
        ...
        # Target label semantics (forward-window): eligible if event hasn't
        # triggered AND we have future; realized = 1 if event triggers in
        # snap_year < trig <= horizon.
        for ev in TARGETS:
...
def main():
    ...
    ap.add_argument("--out-model", default="models/hazards_v1.20h.pkl")
    ap.add_argument("--out-fit-long", default="v1.20h_fit_long.csv")
    ap.add_argument("--out-val-long", default="v1.20h_val_long.csv")
    args = ap.parse_args()
    ...
    print(f"Computing per-level first-year lookups...")
    level_triggers: dict[str, dict[str, int]] = {}
    for tier, levels in LEVEL_SETS.items():
        level_triggers[tier] = _first_year_at_levels(args.db, levels)
        print(f"  {tier:>3}: {len(level_triggers[tier]):,} players ever reached")
    ...
    print(f"\nComputing per-row labels for 5 hazards + 4 targets...")
    labels = _per_row_labels(joined, years, level_triggers,
                              last_active_by_pid)
    for ev in HAZARDS + TARGETS:
    ...
    print(f"\nTraining 5 hazards on 80% train slice...")
    ...
    for ev in HAZARDS:
        ...
        print(f"  [{ev}] n_train={n:,}  pos={n_pos:,} ({n_pos/n:.2%} base)")

    os.makedirs(os.path.dirname(args.out_model) or ".", exist_ok=True)
    with open(args.out_model, "wb") as fh:
        pickle.dump({"hazards": hazards,
                     "events": list(hazards.keys()),
                     "version": "v1.20", "n_features": N_FEATURES,
                     "level_sets": {k: sorted(v)
                                    for k, v in LEVEL_SETS.items()}},
                    fh, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"\nWrote {args.out_model}")

    print(f"\nScoring panel rows out-of-fold for fit + val slices...")
    p_by_ev: dict[str, np.ndarray] = {}
    for ev in HAZARDS:
        if ev not in hazards:
            p_by_ev[ev] = np.full(len(years), np.nan, dtype=np.float32)
            continue
        p_by_ev[ev] = (hazards[ev]["hazard"]
                       .predict_proba(X)[:, 1].astype(np.float32))
