"""v1.19: 4 lasso-logit targets on top of the 3 base hazards.

Targets : TOP_100_PROSPECT, MLB_DEBUT, ESTABLISHED_MLB, All_Star_Plus
Features: p_Minors, p_MLB_service, p_All_Star,
          age_at_snap_centered, years_in_pro,
          p_Minors_x_yip_centered, p_MLB_service_x_yip_centered,
...
TARGETS = ["TOP_100_PROSPECT", "MLB_DEBUT", "ESTABLISHED_MLB",
           "All_Star_Plus"]
HAZARD_PROBS = ["p_Minors", "p_MLB_service", "p_All_Star"]
FEAT = (
    HAZARD_PROBS
    + ["age_at_snap_centered", "years_in_pro"]
...
def fit_one(df_fit: pd.DataFrame, df_val: pd.DataFrame, event: str,
            max_entry: int = 2020, seed: int = 42) -> dict | None:
...
def main():
    ...
    ap.add_argument("--fit", default="v1.19h_fit_long.csv")
    ap.add_argument("--val", default="v1.19h_val_long.csv")
    ...
    bundle = {
        ...
        "age_center": AGE_CENTER, "yip_center": YIP_CENTER,
        "version": "v1.19",
        "kind": "lasso_logit_per_event_3hazard",
        "note": "3 base hazards (Minors, MLB_service, All_Star) → 4 lasso-logits.",
    }
