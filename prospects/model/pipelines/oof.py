...
from prospects.model.hazards.landmark import (
...
def _purge_derived() -> None:
    ...
    Worse than plain staleness, a killed run leaves a MIXED set — some folds
    from the new panel, some from the old — which no amount of reading the
    headline would reveal.
...
def main():
    ...
    if XGB_OUT.exists():
        ...
        tmp = str(XGB_OUT) + ".tmp"
        rc = subprocess.run([
            sys.executable, "-m", "prospects.model.train.joint_xgb",
            "--fit", str(OOF_STACKED),
            "--val", str(OOF_VAL),
            "--db", args.db,
