...
landmark-trained hazards via `prospects.model.hazards.landmark`.
...
Usage:
    python -m prospects.model.pipelines.stage_a

    # Reuse a previously-built landmark panel + hazards (skip retraining):
    python -m prospects.model.pipelines.stage_a --skip-hazards

    # Skip scoring (use existing landmark fit/val CSVs):
    python -m prospects.model.pipelines.stage_a --skip-score

    # Skip downstream refit (just produce the long CSVs):
    python -m prospects.model.pipelines.stage_a --skip-downstream
"""
...
from prospects.model.hazards.survival import (
...
def main():
    ...
    print(f"\n[5/6 ctd] Refitting lasso_logits bundle -> {BUNDLE_OUT.name}",
          flush=True)
    rc = subprocess.run(
        [sys.executable, "-m", "prospects.model.train.lasso_logits",
    ...
    rc = subprocess.run(
        [sys.executable, "-m", "prospects.model.train.time_to_debut",
         "--fit", str(LM_COMBINED_LONG),
         "--val", str(LM_COMBINED_LONG),
         "--db",  args.db,
